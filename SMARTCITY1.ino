...
void procesarEstadoLow() {
    switch (estadoP1) {
        case 0:
            // Ambas condiciones son LOW
            digitalWrite(LR1, LOW);
            digitalWrite(LG1, HIGH);
            digitalWrite(LR2, HIGH);
            tiempoAnterior = millis(); // Guardar el tiempo actual
            estadoP1 = 1; // Cambiar al siguiente estado
            break;

        case 1:
            // Enciende LY1 después de 10 segundos
            if (millis() - tiempoAnterior >= 10000) {
                digitalWrite(LG1, LOW);
                digitalWrite(LY1, HIGH);
                tiempoAnterior = millis(); // Guardar el tiempo actual
                estadoP1 = 2; // Cambiar al siguiente estado
            }
            break;

        case 2:
            // Apaga LY1 después de 1 segundo
            if (millis() - tiempoAnterior >= 1000) {
                ...
                digitalWrite(LG2, HIGH);
                tiempoAnterior = millis(); // Guardar el tiempo actual
                estadoP1 = 3; // Cambiar al siguiente estado
            }
            break;

        case 3:
            // Enciende LY2 después de 10 segundos
            if (millis() - tiempoAnterior >= 10000) {
                digitalWrite(LG2, LOW);
                digitalWrite(LY2, HIGH);
                tiempoAnterior = millis(); // Guardar el tiempo actual
                estadoP1 = 4; // Cambiar al siguiente estado
            }
            break;

        case 4:
            // Apaga LY2 después de 1 segundo y vuelve al estado inicial
            if (millis() - tiempoAnterior >= 1000) {
                digitalWrite(LY2, LOW);
                digitalWrite(LR1, LOW);
                estadoP1 = 0; // Volver al estado inicial
            }
            break;
    }
}
...
void loop() {
    readComm(); // Esperar comando desde Serial para prueba
    checkDigitalIn(); // Verificar si alguna entrada digital está activada

    switch (digitalRead(P1)) {
        case LOW:
            procesarEstadoLow();
            break;

        case HIGH:
            // Lógica para el caso HIGH
            Serial.println("P1 está en HIGH");
            estadoP1 = 1;
            procesarEstadoLow();
            break;
    }
